    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        if (socialLoginProfile.equals("prod")) {
            if (role.equals(String.valueOf(AuthRole.ROLE_USER_A))) {
                response.sendRedirect("http://172.16.211.54/auth?access=" + accessToken + "&signedIn=false");
            } else {
                authService.saveRefreshToken(providerName, "Bearer " + refreshToken);
                response.sendRedirect("http://172.16.211.54/auth?access=Bearer " + accessToken + "&refresh=Bearer " + refreshToken + "&userId=" + providerName + "&signedIn=true");
            }
